            ...
            for correo in rows:
                try:
                    ...
                    if resultado is True:
                        ...
                        self._marcar_correo_no_enviado(
                            correo["IDCorreo"], "Error envío"
                        )
                except (
                    TransientEmailSendError
                ) as e:  # fallo transitorio -> reintentar en ciclo futuro
                    logger.warning(
                    ...
                    logger.error(
                        "Error procesando correo ID %s: %s", correo.get("IDCorreo"), e
                    )
                    self._marcar_correo_no_enviado(correo.get("IDCorreo", -1), str(e))
            return enviados
        except Exception as e:
            logger.error(
                "Error general en process_pending_emails (%s): %s", self.email_source, e
            )
            return enviados
