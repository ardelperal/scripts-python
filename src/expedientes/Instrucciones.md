De acuerdo, he analizado el script VBS que has proporcionado. A continuación, te presento un prompt detallado y estructurado, diseñado para que un modelo de lenguaje como el de Anthropic (Claude) pueda refactorizar tu código a Python de manera efectiva, incorporando las mejores prácticas modernas.

---
¡Entendido! Quieres un desglose de la lógica de negocio pura, en forma de pseudocódigo, para que el modelo de IA pueda entender *qué* debe hacer el nuevo sistema, basándose en el comportamiento del antiguo.

Aquí tienes el pseudocódigo que detalla la lógica de negocio del script `TareaExpedientes.vbs`.

---

### **Lógica de Negocio en Pseudocódigo para Tarea de Informes de Expedientes**

**OBJETIVO DEL PROGRAMA:**
Generar y registrar un informe diario que resalte varios estados críticos o de alerta sobre los expedientes gestionados, basándose en un conjunto de reglas de negocio específicas. El informe se guarda en una base de datos para su posterior envío.

---

**PROCEDIMIENTO PRINCIPAL (`Lanzar`)**

1.  **INICIO**
2.  Establecer conexión con la **Base de Datos de TAREAS**.
3.  Llamar a la función **VerificarSiTareaYaFueRealizada**.
    *   **SI** la función devuelve `VERDADERO`:
        *   Registrar un mensaje: "La tarea diaria ya ha sido completada".
        *   **FINALIZAR PROGRAMA**.
    *   **SI NO** (la tarea debe ejecutarse):
        *   Establecer conexión con la **Base de Datos de EXPEDIENTES**.
        *   Llamar a la función **RealizarProcesoDeInformes**.
        *   Llamar a la función **MarcarTareaComoCompletada** en la DB de TAREAS.
        *   Registrar un mensaje: "La tarea se ha completado y el informe ha sido registrado".
4.  Cerrar todas las conexiones a las bases de datos.
5.  **FIN**

---

**SUB-PROCEDIMIENTOS Y LÓGICA DE NEGOCIO**

**Función: `VerificarSiTareaYaFueRealizada`**

*   **Lógica:** Comprobar si la tarea llamada 'ExpDiario' ya se ha ejecutado con éxito en la fecha actual.
*   **Pasos:**
    1.  Consultar en la tabla `TbTareas` la fecha de la última ejecución (`Ultima`) donde `Tarea` sea 'ExpDiario' y `Realizado` sea 'Sí'.
    2.  Comparar la fecha `Ultima` obtenida con la fecha del día de `HOY`.
    3.  **SI** las fechas coinciden, retornar `VERDADERO`.
    4.  **SI NO**, retornar `FALSO`.

---

**Función: `RealizarProcesoDeInformes`**

*   **Lógica:** Orquestar la recopilación de datos, la generación del informe HTML y su registro.
*   **Pasos:**
    1.  **Recopilar Datos para el Informe:**
        *   Invocar las siguientes 6 funciones para obtener los datos de expedientes que cumplen ciertas condiciones. Cada función ejecuta una consulta SQL específica en la DB de EXPEDIENTES y devuelve una colección de registros.

        a.  **Obtener Expedientes a Punto de Finalizar:**
            *   **Regla de Negocio:** Seleccionar expedientes cuyo contrato (`FechaFinContrato`) esté a 14 días o menos de finalizar (pero sin haber pasado ya).
            *   Datos a obtener: ID, Código, Título, Fechas de contrato, Responsable, Días restantes, etc.

        b.  **Obtener Hitos a Punto de Finalizar:**
            *   **Regla de Negocio:** Seleccionar hitos de expedientes (`FechaHito`) que estén a 14 días o menos de su fecha de cumplimiento.
            *   Datos a obtener: ID del Expediente, Código, Título, Descripción del Hito, Fecha del Hito, Días restantes.

        c.  **Obtener Expedientes con Estado Desconocido:**
            *   **Regla de Negocio:** Seleccionar todos los expedientes cuyo campo `Estado` sea exactamente 'Desconocido'.
            *   Datos a obtener: ID, Código, Título, Fechas, Responsable.

        d.  **Obtener Expedientes Adjudicados sin Datos de Contrato:**
            *   **Regla de Negocio:** Seleccionar expedientes que tengan una `FECHAADJUDICACION` pero no tengan `FechaInicioContrato`, `FechaFinContrato` ni `GARANTIAMESES`. Esto indica una adjudicación a la que no se le han cargado los detalles del contrato.
            *   Datos a obtener: ID, Código, Título, Fecha de Adjudicación, Responsable.

        e.  **Obtener Expedientes TSOL Adjudicados sin Código S4H:**
            *   **Regla de Negocio:** Seleccionar expedientes adjudicados que pertenezcan a la entidad jurídica 'TSOL' pero cuyo campo `CodS4H` esté vacío (nulo). Se excluyen aquellos marcados explícitamente para no aplicar esta tarea.
            *   Datos a obtener: ID, Código, Título, Responsable, Jurídica (TSOL), Fecha de Adjudicación.

        f.  **Obtener Expedientes en Fase de Oferta por Mucho Tiempo:**
            *   **Regla de Negocio:** Seleccionar expedientes que tienen una `FECHAOFERTA`, pero han pasado 45 días o más desde esa fecha sin que se haya registrado una `FECHAADJUDICACION`, `FECHAPERDIDA` o `FECHADESESTIMADA`.
            *   Datos a obtener: ID, Código, Título, Fecha de Oferta, Responsable.

    2.  **Generar el Cuerpo del Informe (HTML):**
        *   Leer un archivo `CSS.txt` externo para obtener los estilos.
        *   Crear la estructura base de un documento HTML e incrustar el CSS en la cabecera.
        *   Para cada una de las 6 colecciones de datos obtenidas en el paso anterior:
            *   Si la colección contiene datos, generar una tabla HTML.
            *   La tabla debe tener un título descriptivo (ej: "EXPEDIENTES A PUNTO DE RECEPCIONAR/FINALIZAR").
            *   La primera fila de la tabla debe contener los encabezados de las columnas.
            *   Iterar sobre cada registro de la colección y crear una fila (`<tr>`) en la tabla con sus datos.
        *   Combinar todas las tablas HTML generadas en un solo bloque de texto HTML, separadas por espacios.

    3.  **Registrar el Correo para Envío:**
        *   Obtener la lista de correos de los "Tramitadores" de la base de datos de TAREAS.
        *   Llamar a la función **RegistrarCorreoEnBaseDeDatos**, pasándole:
            *   **Asunto:** "Informe Tareas De Expedientes (Expedientes)".
            *   **Cuerpo:** El texto HTML completo generado en el paso anterior.
            *   **Destinatarios:** La lista de correos de los tramitadores.

---

**Función: `RegistrarCorreoEnBaseDeDatos`**

*   **Lógica:** Almacenar el informe generado en una tabla para que otro proceso pueda enviarlo.
*   **Pasos:**
    1.  Calcular un nuevo `IDCorreo` (normalmente el máximo ID existente + 1).
    2.  Insertar un nuevo registro en la tabla `TbCorreosEnviados` con los siguientes campos: `IDCorreo`, `Aplicacion` ('EXPEDIENTES'), `Asunto`, `Cuerpo` (el HTML), `Destinatarios`, `FechaGrabacion` (`AHORA`).

---

**Función: `MarcarTareaComoCompletada`**

*   **Lógica:** Actualizar el estado de la tarea 'ExpDiario' para evitar que se ejecute de nuevo en el mismo día.
*   **Pasos:**
    1.  Buscar en `TbTareas` el registro donde `Tarea` es 'ExpDiario'.
    2.  **SI** existe:
        *   Actualizar su `Fecha` a la de `HOY` y `Realizado` a 'Sí'.
    3.  **SI NO** existe:
        *   Insertar un nuevo registro con `Tarea`='ExpDiario', `Fecha`=`HOY` y `Realizado`='Sí'.



A continuación, se adjunta el código VBS original completo para su análisis y refactorización.

```vb
--- INICIO DEL ARCHIVO TareaExpedientes.txt ---

[Aquí pegarías el contenido completo del archivo TareaExpedientes.txt que me proporcionaste]
...
--- FIN DEL ARCHIVO TareaExpedientes.txt ---
```