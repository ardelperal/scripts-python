...
from common.db.access_connection_pool import get_expedientes_connection_pool
...
class ExpedientesTask(TareaDiaria):
    ...
    def execute_specific_logic(self) -> bool:
        ...
        try:
            self.logger.info(
                "Inicio lógica específica Expedientes",
                extra={"event": "expedientes_task_logic_start", "app": "EXPEDIENTES"},
            )
            ...
            manager = ExpedientesManager(
                self.db_expedientes, self.db_tareas, logger=self.logger
            )
            ...
            self.logger.info(
                ...
                extra={
                    "event": "expedientes_task_logic_end",
                    "success": bool(result),
                    "app": "EXPEDIENTES",
                },
            )
            ...
            self.logger.error(
                f"Error en execute_specific_logic Expedientes: {e}",
                extra={"context": "execute_specific_logic"},
            )
            return False
