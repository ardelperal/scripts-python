...
class ExpedientesManager:
    ...
    def __init__(
        self,
        db_expedientes: AccessDatabase,
        db_tareas: AccessDatabase,
    ...
    def get_expedientes_adjudicados_sin_contrato(self) -> list[dict]:
        ...
        Basado en la función getColAdjudicadosSinContrato() del script original
        TareaExpedientes.vbs
        """
        try:
            db_expedientes = self.db_expedientes
    ...
    # _build_table_html eliminado en favor de build_table_html común
    ...
    def generate_expedientes_report_html(self) -> str:
        ...
        Logging estructurado para Grafana/Loki:
          - event: 'expedientes_report_start' / 'expedientes_report_empty' /
            'expedientes_report_section' / 'expedientes_report_summary'
          - metric_name / metric_value: pares genéricos ya utilizados en el ecosistema
          - section: slug de la sección (sin espacios ni acentos) para facilitar
            agregaciones
        """
        try:
            self.logger.info(
            ...
            sections = [
                (
                    "Expedientes TSOL adjudicados sin código S4H",
                    self.get_expedientes_tsol_sin_cod_s4h(),
                ),
                (
                    "Expedientes a punto de finalizar",
                    self.get_expedientes_a_punto_finalizar(),
                ),
                ("Hitos a punto de finalizar", self.get_hitos_a_punto_finalizar()),
                (
                    "Expedientes con estado desconocido",
                    self.get_expedientes_estado_desconocido(),
                ),
                (
                    "Expedientes adjudicados sin contrato",
                    self.get_expedientes_adjudicados_sin_contrato(),
                ),
                (
                    "Expedientes en fase oferta > 45 días",
                    self.get_expedientes_fase_oferta_mucho_tiempo(),
                ),
            ]
            ...
            parts = [
                self.html_generator.generar_header_moderno(
                    "INFORME DE AVISOS DE EXPEDIENTES"
                )
            ]
            total_rows = 0
...
def main():
    ...
    try:
        ...
        logger.warning(
            "Instancias reales de AccessDatabase no configuradas; usando None (sólo "
            "HTML vacío)"
        )
        ...
        if manager:
            try:
                ...
                logger.warning(
                    f"Error cerrando conexiones: {e}", extra={"context": "main_close"}
                )
