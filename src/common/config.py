...
class Config:
    ...
    def __init__(self):
        ...
        # Cada entrada define: variable entorno local, variable entorno office, ruta local por defecto (relativa a root) y ruta office por defecto.
        ...
        for name, data in self._db_definitions.items():
            if self.environment == 'local':
                ...
                path = self.root_dir / raw if not raw.startswith('\\\\') else Path(raw)  # permitir rutas absolutas de red en local si se proporcionan
            else:
    ...
    def get_local_db_path(self, db_type: str) -> Path:  # pragma: no cover - lógica sencilla
