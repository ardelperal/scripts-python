...
from .base_task import TareaContinua, TareaDiaria
...
try:
    ...
    from src.no_conformidades.no_conformidades_task import (
        NoConformidadesTask,  # type: ignore
    )
    try:
        from src.riesgos.riesgos_task import RiesgosTask  # type: ignore
    except ModuleNotFoundError:
        RiesgosTask = None  # type: ignore
except ModuleNotFoundError:
    ...
    from no_conformidades.no_conformidades_task import (
        NoConformidadesTask,  # type: ignore
    )
    try:
...
# Stub de RiesgosTask si no existe implementación real (para mantener compatibilidad en tests)
if 'RiesgosTask' in globals() and RiesgosTask is None:
    class RiesgosTask(TareaDiaria):  # type: ignore
        def __init__(self):  # pragma: no cover - comportamiento mínimo
            super().__init__(
                name="Riesgos",
                script_filename="run_riesgos.py",
                task_names=["RiesgosDiario"],
                frequency_days=1,
            )
...
class NoConformidadesTask(TareaDiaria):
    ...
    def __init__(self):
        ...
        super().__init__(
            ...
            task_names=["NoConformidadesCalidad", "NoConformidadesTecnica"],
        ...
        try:
            ...
            for task_name in self.task_names:
                ...
                if should_execute_task(
                    self.db_tareas, task_name, frequency, self.logger
                ):
                    self.logger.info(
                        f"📅 {task_name} debe ejecutarse (frecuencia: {frequency} días)"
                    )
                    return True
...
class EmailServicesRegistryTask(TareaContinua):
    ...
    def __init__(self):
        super().__init__(name="EmailServices", script_filename="run_email_services.py")
    ...
    def execute_specific_logic(self) -> bool:  # pragma: no cover - mínima
        ...
        if mode == "direct":
            try:
                from src.email_services.email_task import (
                    EmailServicesTask,  # import local para evitar ciclos
                )
...
class TaskRegistry:
    ...
    def __init__(
        ...
        if include_daily:
            base_daily = [BrassTask(), ExpedientesTask(), NoConformidadesTask(), AgedysTask()]
            if 'RiesgosTask' in globals() and RiesgosTask is not None:  # type: ignore
                try:
                    base_daily.insert(0, RiesgosTask())  # Riesgos primero como antes
                except Exception:
                    pass
            self._daily_tasks.extend(base_daily)
        if include_continuous:
            self._continuous_tasks.extend([EmailServicesRegistryTask()])
        if extra_daily:
...
__all__ = [
    ...
    "get_all_tasks",
    # Clases de tareas
    # RiesgosTask puede no estar disponible
    *(["RiesgosTask"] if 'RiesgosTask' in globals() and RiesgosTask is not None else []),
    "BrassTask",
    "ExpedientesTask",
    "NoConformidadesTask",
    "AgedysTask",
