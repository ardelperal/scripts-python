...
class TareaDiaria(BaseTask):
    ...
    def __init__(self, name: str, script_filename: str, task_names: List[str], frequency_days: int = 1):
        """
    ...
    def debe_ejecutarse(self) -> bool:
        ...
        try:
            ...
            for task_name in self.task_names:
                if should_execute_task(self.db_tareas, task_name, self.frequency_days, self.logger):
    ...
    def marcar_como_completada(self):
        ...
        """
        if not self.db_tareas:
            self.logger.warning(f"⚠️  No hay conexión a BD, no se puede registrar {self.name}")
            return
