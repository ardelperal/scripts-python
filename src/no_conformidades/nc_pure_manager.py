...
NOTA: Refactor incremental: sólo se incluyen algunas secciones iniciales (ARs próximas a vencer y NCs pendientes de control eficacia). El resto de secciones legacy permanecen en el manager antiguo hasta completar migración.
"""
...
class NoConformidadesManagerPure:
    ...
    def get_ars_proximas_vencer_calidad(self) -> List[Dict[str, Any]]:
        try:
            ...
            self.logger.info(
                ...
                extra={'event': 'nc_section_fetch', 'section': 'ars_proximas_calidad', 'rows': len(rows)}
            )
            return rows or []
    ...
    def get_ncs_pendientes_eficacia(self) -> List[Dict[str, Any]]:
        try:
            ...
            self.logger.info(
                ...
                extra={'event': 'nc_section_fetch', 'section': 'ncs_pendientes_eficacia', 'rows': len(rows)}
            )
            return rows or []
        except Exception as e:  # pragma: no cover
    ...
    def get_ncs_sin_acciones(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                "NCs sin acciones",
                extra={'event': 'nc_section_fetch', 'section': 'ncs_sin_acciones', 'rows': len(rows)}
            )
            return rows or []
        except Exception as e:  # pragma: no cover
    ...
    def get_ars_para_replanificar(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                ...
                extra={'event': 'nc_section_fetch', 'section': 'ars_para_replanificar', 'rows': len(rows)}
            )
            return rows or []
        except Exception as e:  # pragma: no cover
    ...
    def generate_nc_report_html(self) -> str:
        ...
        if not non_empty:
            self.logger.info(
                "Informe NC parcial vacío",
                extra={'event': 'nc_report_empty'}
            )
            return ""
        parts: list[str] = []
        parts.append(self.html_generator.generar_header_moderno(
        ...
        for title, data in non_empty:
            ...
            self.logger.info(
                ...
                extra={'event': 'nc_report_section', 'section': title.lower().replace(' ', '_'), 'metric_name': 'nc_section_rows', 'metric_value': len(data), 'app': 'NC'}
            )
        html = ''.join(parts) + self.html_generator.generar_footer_moderno()
        self.logger.info(
            "Resumen informe NC parcial",
            extra={'event': 'nc_report_summary', 'metric_name': 'nc_report_sections', 'metric_value': len(non_empty), 'total_rows': total_rows, 'html_length': len(html), 'app': 'NC'}
        )
        self.logger.info(
            "Longitud informe NC parcial",
            extra={'event': 'nc_report_length', 'metric_name': 'nc_report_length_chars', 'metric_value': len(html), 'app': 'NC'}
        )
        return html
