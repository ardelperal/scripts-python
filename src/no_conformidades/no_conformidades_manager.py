...
class NoConformidadesManager(TareaDiaria):
    ...
    def get_ars_proximas_a_vencer_calidad(self) -> List[Dict]:
        """
        Obtiene las ARs de Calidad próximas a vencer (1-15 días).
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
                ...
                AND ac.FechaFinReal IS NULL 
                AND ac.FechaFinPrevista IS NOT NULL
                AND (ac.FechaFinPrevista - Date()) BETWEEN 1 AND 15
                ORDER BY ac.FechaFinPrevista
            """
            return db_nc.execute_query(query)
        except Exception as e:
            self.logger.error(f"Error obteniendo ARs de calidad próximas a vencer: {e}")
            return []

    def get_ars_vencidas_calidad(self) -> List[Dict]:
        """
        Obtiene las ARs de Calidad vencidas.
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
            ...
            self.logger.error(f"Error obteniendo ARs de calidad vencidas: {e}")
            return []

    def get_ars_proximas_vencer_calidad(self) -> List[Dict]:
        """
        Obtiene las ARs próximas a caducar o caducadas (sin fecha fin real) para calidad.
        Basado en la consulta SQL 1.1.a de las reglas del proyecto.
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
    ...
    def get_ncs_pendientes_eficacia(self) -> List[Dict]:
        """
        Alias para get_nc_pendientes_eficacia para compatibilidad con el script.
        Obtiene No Conformidades resueltas pendientes de Control de Eficacia.
        Basado en la consulta SQL 1.1.b de las reglas del proyecto.
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
    ...
    def get_ncs_sin_acciones(self) -> List[Dict]:
        """
        Alias para get_nc_registradas_sin_acciones para compatibilidad con el script.
        Obtiene No Conformidades sin Acciones Correctivas registradas.
        Basado en la consulta SQL 1.1.c de las reglas del proyecto.
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
    ...
    def get_ars_para_replanificar(self) -> List[Dict]:
        """
        Obtiene ARs para Replanificar (fecha prevista cercana o pasada, sin fecha fin real).
        Basado en la consulta SQL 1.1.d de las reglas del proyecto.
        """
        try:
            db_nc = self._get_nc_connection()
            query = """
