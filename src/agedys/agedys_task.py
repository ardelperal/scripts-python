...
class AgedysTask(TareaDiaria):
    ...
    def execute_specific_logic(self) -> bool:
        ...
        try:
            ...
            recipients_service = EmailRecipientsService(
            ...
            def _handle_technical_reports():
                ...
                self.logger.info(
                    ...
                    extra={
                        "event": "agedys_users_pending_fetch",
                        "users": len(usuarios),
                    },
                )
                for u in usuarios:
                    ...
                    user_name = (
                        u.get("UserName") or u.get("user_name") or u.get("Nombre") or ""
                    )
                    user_email = (
                        u.get("UserEmail")
                        or u.get("user_email")
                        or u.get("CorreoUsuario")
                        or ""
                    )
                    if user_id is None or not user_email:
                        self.logger.debug(
                            ...
                            extra={
                                "event": "agedys_user_skipped",
                                "reason": "missing_id_or_email",
                                "user_id": user_id,
                        ...
                        continue
                    try:
                        html = manager.generate_technical_user_report_html(
                        ...
                        self.logger.error(
                            ...
                            extra={
                                "event": "agedys_user_report_error",
                                "user_id": user_id,
                                "error": str(ex),
                            },
                        )
                    ...
                    if not html:
                        self.logger.debug(
                            ...
                            extra={
                                "event": "agedys_user_report_empty",
                                "user_id": user_id,
                            },
                        )
                        continue
                    subject = (
                        f"Informe Tareas Pendientes (AGEDYS) - {user_name}".strip()
                    )
                    try:
                        ...
                        self.logger.error(
                            ...
                            extra={
                                "event": "agedys_user_report_register_error",
                                "user_id": user_id,
                                "error": str(ex),
                            },
                        )
                self.logger.info(
                    ...
                    extra={
                        "event": "agedys_technical_reports_done",
                        "registered": count,
                    },
                )
                return count
            ...
            def _handle_quality_report():
                ...
                self.logger.info(
                    ...
                    extra={
                        "event": "agedys_quality_debug_marker",
                        "app_id": app_id_num,
                    },
                )
                quality_emails = recipients_service.get_quality_emails(
                    app_id=app_id_num
                )
            ...
            self.logger.info(
                ...
                extra={
                    "event": "agedys_task_logic_end",
                    "success": True,
                    "registered_reports": total_registered,
                    "app": "AGEDYS",
                },
            )
            return True
            ...
            self.logger.error(
                f"Error en execute_specific_logic AGEDYS: {e}",
                extra={"context": "execute_specific_logic_agedys"},
            )
            return False
