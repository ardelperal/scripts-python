...
Notas:
    - La sección "usuarios_facturas_pendientes" replica la lógica legacy compuesta de 4
      consultas unificadas por UsuarioRed.
    - Consultas marcadas como 'Simplificado' deben ajustarse si el esquema real difiere;
      placeholders iniciales para completar en siguientes iteraciones.
...
class AgedysPureManager:
    ...
    def get_usuarios_facturas_pendientes_visado_tecnico(self) -> list[dict[str, Any]]:
        ...
        Se unifican resultados en un diccionario para evitar duplicados y luego se
        devuelve lista.
        """
        queries = [
            (
        ...
        for code, sql in queries:
            try:
                ...
                self.logger.error(
                    f"Error subconsulta {code} usuarios_facturas_pendientes: {e}"
                )
                rows = []
            ...
            self.logger.info(
                ...
                extra={
                    ...
                    "section": "usuarios_facturas_pendientes",
                    "subquery": code,
                    "rows": len(rows or []),
                },
            )
