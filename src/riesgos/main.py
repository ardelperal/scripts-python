...
def main():
    ...
    parser = argparse.ArgumentParser(description='Gestión de Riesgos - Informes Automatizados')
    parser.add_argument('--technical', action='store_true', 
                       help='Ejecutar informes técnicos semanales')
    parser.add_argument('--quality', action='store_true',
                       help='Ejecutar informes de calidad semanales')
    parser.add_argument('--monthly', action='store_true',
                       help='Ejecutar informe mensual de calidad')
    ...
    try:
        ...
        success = any(results.values()) if any([args.technical, args.quality, args.monthly]) else all(results.values())
        ...
        if stats['has_errors']:
            logger.error(f"🚨 SE DETECTARON {stats['error_count']} ERRORES DURANTE LA EJECUCIÓN")
            logger.error("Revisa el archivo de logs de errores SQL para más detalles")
        
        if stats['has_warnings']:
            logger.warning(f"⚠️ SE DETECTARON {stats['warning_count']} WARNINGS DURANTE LA EJECUCIÓN")
