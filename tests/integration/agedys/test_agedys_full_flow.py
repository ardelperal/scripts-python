...
def test_full_quality_report_flow():
    ...
    assert (
        "ResponsableCalidad" in html or "Responsable Calidad" in html
    ) and "RC" in html
...
def test_full_economy_report_flow():
    ...
    manager.get_dpds_sin_pedido_agrupado = Mock(
        return_value=[
            {
                "CODPROYECTOS": "D2",
                "PETICIONARIO": "Tec",
                "FECHAPETICION": "2025-01-02",
                "EXPEDIENTE": "E2",
                "DESCRIPCION": "Desc2",
            }
        ]
...
def test_full_technical_user_report_multiple_sections():
    manager, db = build_manager()
    manager.get_dpds_sin_so_por_tecnico = Mock(
        return_value=[
            {
                "CODPROYECTOS": "D1",
                "PETICIONARIO": "Tec",
    ...
    manager.get_dpds_con_so_sin_ro_por_tecnico = Mock(
        return_value=[
            {
                "CODPROYECTOS": "D2",
                "PETICIONARIO": "Tec",
