...
class TestAgedysDatabaseIntegration:
    ...
    def verify_local_environment(self):
        ...
        if missing_dbs:
            pytest.skip(
                f"Bases de datos locales no encontradas: {', '.join(missing_dbs)}"
            )
    ...
    def test_agedys_queries_real_data(self, local_agedys_manager):
        ...
        try:
            usuarios_facturas = (
                local_agedys_manager.get_usuarios_facturas_pendientes_visado_tecnico()
            )
            ...
            if usuarios_facturas:
                ...
                for field in required_fields:
                    assert (
                        field in primer_usuario
                    ), f"Campo '{field}' faltante en resultado"

        except Exception as e:
            pytest.fail(
                f"Error en get_usuarios_facturas_pendientes_visado_tecnico: {e}"
            )

        # Test 2: Usuarios con DPDs sin visado de calidad
        try:
            usuarios_dpds_sin_visado = (
                local_agedys_manager.get_usuarios_dpds_sin_visado_calidad()
            )
        ...
        try:
            usuarios_dpds_rechazadas = (
                local_agedys_manager.get_usuarios_dpds_rechazados_calidad()
            )
    ...
    def test_additional_agedys_queries_real_data(self, local_agedys_manager):
        ...
        try:
            ...
            usuarios_dpds_pendientes = (
                local_agedys_manager.get_usuarios_dpds_pendientes_recepcion_economica()
            )
            assert isinstance(usuarios_dpds_pendientes, list), "Debe devolver una lista"
            print(
                f"✓ get_usuarios_dpds_pendientes_recepcion_economica: {len(usuarios_dpds_pendientes)} usuarios"
            )
            ...
            if usuarios_dpds_pendientes:
                ...
                dpds_pendientes = (
                    local_agedys_manager.get_dpds_pendientes_recepcion_economica(
                        usuario_test
                    )
                )
                assert isinstance(dpds_pendientes, list), "Debe devolver una lista"
                print(
                    f"✓ get_dpds_pendientes_recepcion_economica para {usuario_test}: {len(dpds_pendientes)} DPDs"
                )
            else:
            ...
            dpds_fin_agenda = (
                local_agedys_manager.get_dpds_fin_agenda_tecnica_por_recepcionar()
            )
            assert isinstance(dpds_fin_agenda, list), "Debe devolver una lista"
            print(
                f"✓ get_dpds_fin_agenda_tecnica_por_recepcionar: {len(dpds_fin_agenda)} DPDs"
            )
            ...
            if len(usuarios_dpds_pendientes) > 0:
                ...
                facturas = local_agedys_manager.get_facturas_pendientes_visado_tecnico(
                    usuario
                )
                html_facturas = local_agedys_manager.generate_facturas_html_table(
                    facturas
                )
                assert isinstance(html_facturas, str), "HTML debe ser una cadena"
                print(
                    f"✓ generate_facturas_html_table: {len(html_facturas)} caracteres"
                )

            # Test generate_dpds_html_table
            if len(dpds_sin_pedido) > 0:
                html_dpds = local_agedys_manager.generate_dpds_html_table(
                    dpds_sin_pedido, "sin_pedido"
                )
    ...
    def test_notification_registration_methods_dry_run(self, local_agedys_manager):
        ...
        try:
            ...
            if usuarios_facturas:
                ...
                local_agedys_manager.register_facturas_pendientes_notification(
                    usuario, email, facturas, dry_run=True
                )
            ...
            if usuarios_dpds_sin_visado:
                ...
                local_agedys_manager.register_dpds_sin_visado_notification(
                    usuario, email, dpds, dry_run=True
                )
                print("✓ register_dpds_sin_visado_notification (dry-run)")

            # Test register_dpds_rechazados_notification
            usuarios_dpds_rechazados = (
                local_agedys_manager.get_usuarios_dpds_rechazados_calidad()
            )
            if usuarios_dpds_rechazados:
                ...
                local_agedys_manager.register_dpds_rechazados_notification(
                    usuario, email, dpds, dry_run=True
                )
            ...
            if usuarios_economia and dpds:
                local_agedys_manager.register_economia_notification(
                    usuarios_economia, dpds, dry_run=True
                )
            ...
            if dpds_sin_pedido:
                ...
                local_agedys_manager.register_dpds_sin_pedido_notification(
                    usuario, email, dpds_sin_pedido, dry_run=True
                )
        ...
        try:
            ...
            if usuarios_facturas:
                ...
                if facturas:
                    ...
                    print(
                        f"✓ Notificación registrada para {usuario} con {len(facturas[:3])} facturas"
                    )
                else:
    ...
    def test_database_connectivity(self, local_agedys_manager):
        ...
        assert (
            local_agedys_manager.db is not None
        ), "Base de datos AGEDYS debe estar disponible"
        assert (
            local_agedys_manager.correos_db is not None
        ), "Base de datos Correos debe estar disponible"

        # Probar consultas básicas
        try:
            # Test en base de datos AGEDYS
            result = local_agedys_manager.db.execute_query(
                "SELECT COUNT(*) as total FROM TbProyectos"
            )
            assert isinstance(result, list) and len(result) > 0
            print(f"✓ TbProyectos: {result[0]['total']} registros")

            # Test en base de datos Correos
            result = local_agedys_manager.correos_db.execute_query(
    ...
    def test_real_usuarios_facturas_pendientes(self, local_agedys_manager):
        ...
        try:
            ...
            if usuarios:
                ...
                print(
                    f"✓ Usuario {usuario['UsuarioRed']}: {len(facturas)} facturas pendientes"
                )
            else:
    ...
    def test_real_usuarios_dpds_sin_visado(self, local_agedys_manager):
        ...
        try:
            ...
            if usuarios:
                ...
                dpds = local_agedys_manager.get_dpds_sin_visado_calidad(
                    usuario["UsuarioRed"]
                )
    ...
    def test_real_usuarios_dpds_rechazados(self, local_agedys_manager):
        ...
        try:
            ...
            if usuarios:
                ...
                # Probar obtener DPDs para el primer usuario
                dpds = local_agedys_manager.get_dpds_rechazados_calidad(
                    usuario["UsuarioRed"]
                )
                assert isinstance(dpds, list), "DPDs debe ser una lista"
    ...
    def test_data_consistency(self, local_agedys_manager):
        ...
        for usuario_info in usuarios_facturas:
            ...
            assert (
                len(facturas) > 0
            ), f"Usuario {usuario} aparece en lista pero no tiene facturas"
            ...
            for factura in facturas:
                ...
                assert (
                    "CODPROYECTOS" in factura
                ), "Factura debe tener código de proyecto"
