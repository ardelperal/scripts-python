...
class TestRiesgosDatabaseIntegration:
    ...
    def test_database_connectivity(self, local_riesgos_manager):
        ...
        result_riesgos = local_riesgos_manager.db.execute_query("SELECT COUNT(*) as total FROM TbRiesgos")
        ...
        result_tareas = local_riesgos_manager.tareas_db.execute_query("SELECT COUNT(*) as total FROM TbTareas")
    ...
    def test_riesgos_structure(self, local_riesgos_manager):
        ...
        result = local_riesgos_manager.db.execute_query("SELECT COUNT(*) as total FROM TbRiesgos")
    ...
    def test_comprehensive_riesgos_query(self, local_riesgos_manager):
        ...
        try:
            ...
            if result:
                ...
                riesgo = result[0]
                expected_fields = ['IDRiesgo', 'CodigoRiesgo', 'Descripcion', 'Nemotecnico']
                for field in expected_fields:
                    assert field in riesgo, f"Campo {field} no encontrado en consulta comprehensiva"
    ...
    def test_riesgos_joins_with_projects(self, local_riesgos_manager):
        ...
        try:
            ...
            simple_query = "SELECT IDRiesgo, Descripcion FROM TbRiesgos WHERE IDRiesgo <= (SELECT MIN(IDRiesgo) + 4 FROM TbRiesgos)"
            try:
                result = local_riesgos_manager.db.execute_query(simple_query)
                assert isinstance(result, list), "El resultado debe ser una lista"
            except Exception:
                # Usar sintaxis compatible con Access
                result = local_riesgos_manager.db.execute_query("SELECT IDRiesgo, Descripcion FROM TbRiesgos")
                assert isinstance(result, list), "El resultado debe ser una lista"