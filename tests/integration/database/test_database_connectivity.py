...
class TestDatabaseConnectivity:
    ...
    def test_environment_configuration(self):
        ...
        assert config.environment in [
            "local",
            "oficina",
        ], f"Entorno no válido: {config.environment}"
    ...
    def test_database_paths_exist(self):
        ...
        if missing_databases:
            pytest.skip(
                f"Bases de datos no encontradas en entorno '{config.environment}': {', '.join(missing_databases)}"
            )
    ...
    def test_brass_database_connection(self):
        ...
        try:
            ...
            if not Path(config.db_brass_path).exists():
                pytest.skip(
                    f"Base de datos BRASS no encontrada: {config.db_brass_path}"
                )
            ...
            with pyodbc.connect(connection_string, timeout=10) as conn:
                ...
                assert (
                    result is not None
                ), "No se pudo ejecutar consulta básica en BD BRASS"
    ...
    def test_tareas_database_connection(self):
        ...
        try:
            ...
            if not Path(config.db_tareas_path).exists():
                pytest.skip(
                    f"Base de datos Tareas no encontrada: {config.db_tareas_path}"
                )
            ...
            with pyodbc.connect(connection_string, timeout=10) as conn:
                ...
                assert (
                    result is not None
                ), "No se pudo ejecutar consulta básica en BD Tareas"
                assert result[0] == 1, "Resultado inesperado en consulta básica"
    ...
    def test_correos_database_connection(self):
        ...
        try:
            ...
            if not Path(config.db_correos_path).exists():
                pytest.skip(
                    f"Base de datos Correos no encontrada: {config.db_correos_path}"
                )
            ...
            with pyodbc.connect(connection_string, timeout=10) as conn:
                ...
                assert (
                    result is not None
                ), "No se pudo ejecutar consulta básica en BD Correos"
                assert result[0] == 1, "Resultado inesperado en consulta básica"
    ...
    def test_correos_database_connection_without_password(self):
        ...
        try:
            ...
            with pyodbc.connect(connection_string, timeout=10) as conn:
                ...
                assert (
                    result is not None
                ), "No se pudo ejecutar consulta básica en BD Correos sin password"
                assert result[0] == 1, "Resultado inesperado en consulta básica"
    ...
    def test_all_databases_summary(self):
            ...
            try:
                ...
                with pyodbc.connect(connection_string, timeout=10) as conn:
                    ...
                    if result and result[0] == 1:
                        results[db_name] = "✅ Conectado correctamente"
                        accessible_count += 1
                    else:
                        results[db_name] = "❌ Consulta falló"
                ...
                if "contraseña" in error_msg.lower() or "password" in error_msg.lower():
                    ...
                    try:
                        ...
                        with pyodbc.connect(conn_str_no_pwd, timeout=10) as conn:
                            ...
                            result = cursor.fetchone()
                            if result and result[0] == 1:
                                results[db_name] = "✅ Conectado sin contraseña"
                                accessible_count += 1
                            else:
                                results[db_name] = "❌ Consulta falló sin contraseña"
                    except pyodbc.Error:
                        results[db_name] = "❌ Requiere contraseña válida"
                else:
        ...
        assert (
            accessible_count > 0
        ), f"Ninguna base de datos está accesible en entorno '{config.environment}'"
        ...
        if config.environment == "oficina":
            ...
            accessible_existing = [
                ...
                if "✅" in status and Path(databases[db]).exists()
            ]

            if len(existing_dbs) > 1 and len(accessible_existing) < len(existing_dbs):
                failed_existing = [
                    db for db in existing_dbs if db not in accessible_existing
                ]
                pytest.fail(
                    f"En entorno oficina, estas BD existentes deberían estar accesibles: {failed_existing}"
                )
