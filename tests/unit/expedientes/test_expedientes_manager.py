...
class TestExpedientesManager(unittest.TestCase):
    def setUp(self):
        ...
        self.manager = ExpedientesManager(
            self.db_exp, self.db_tareas, logger=self.logger
        )
    ...
    def test_get_expedientes_tsol_sin_cod_s4h_error(self):
        ...
        self.assertTrue(
            any(
                k.get("extra", {}).get("context") == "get_expedientes_tsol_sin_cod_s4h"
                for _, k in (
                    (c.args, c.kwargs) for c in self.logger.error.call_args_list
                )
            )
        )
        ...
        mock_rows = [
            {
                ...
                "Dias": 5,
                "FECHACERTIFICACION": date(2024, 1, 2),
                "GARANTIAMESES": 12,
    ...
    def test_get_expedientes_a_punto_finalizar_error(self):
        ...
        self.assertTrue(
            any(
                k.get("extra", {}).get("context") == "get_expedientes_a_punto_finalizar"
                for _, k in (
                    (c.args, c.kwargs) for c in self.logger.error.call_args_list
                )
            )
    ...
    def test_generate_expedientes_report_html_metrics(self):
        self.manager.get_expedientes_tsol_sin_cod_s4h = Mock(
            return_value=[{"IDExpediente": 1, "CodExp": "E1"}]
        )
        self.manager.get_expedientes_a_punto_finalizar = Mock(
            return_value=[{"IDExpediente": 2, "CodExp": "E2"}]
        )
        ...
        metric_calls = [
            ...
            if c.kwargs.get("extra", {}).get("metric_name")
        ]
        metrics = {
            c.kwargs["extra"]["metric_name"]: c.kwargs["extra"]["metric_value"]
