...
class TestExpedientesManagerReport(unittest.TestCase):
    ...
    def test_generate_report_with_some_sections(self):
        with patch.object(self.manager, 'get_expedientes_tsol_sin_cod_s4h', return_value=[{'A': 1}]), \
             patch.object(self.manager, 'get_expedientes_a_punto_finalizar', return_value=[]), \
             patch.object(self.manager, 'get_hitos_a_punto_finalizar', return_value=[{'B': 2}]), \
             patch.object(self.manager, 'get_expedientes_estado_desconocido', return_value=[]), \
             patch.object(self.manager, 'get_expedientes_adjudicados_sin_contrato', return_value=[]), \
             patch.object(self.manager, 'get_expedientes_fase_oferta_mucho_tiempo', return_value=[]):
            html = self.manager.generate_expedientes_report_html()
