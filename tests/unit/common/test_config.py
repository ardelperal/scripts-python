...
def get_current_env_config():
    ...
    if current_env == "local":
        ...
        return {
            ...
            "db_brass": os.getenv(
                "OFFICE_DB_BRASS",
                r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\Gestion_Brass_Gestion_Datos.accdb",
            ),
            "db_tareas": os.getenv(
                "OFFICE_DB_TAREAS",
                r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\Tareas_datos1.accdb",
            ),
            "db_correos": os.getenv(
                "OFFICE_DB_CORREOS",
                r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\correos_datos.accdb",
            ),
            "css_file": os.getenv(
                "OFFICE_CSS_FILE",
                r"\\datoste\Aplicaciones_dys\Aplicaciones PpD\CSS.txt",
            ),
...
class TestConfig:
    ...
    def test_init_current_environment(self):
        ...
        if env_config["environment"] == "local":
            ...
            assert (
                "CSS.txt" in css_path_str
                or "CSS_moderno.css" in css_path_str
                or ".css" in css_path_str
            )
    ...
    def test_init_local_environment_mock(self):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            with patch("common.config.load_dotenv"):
                ...
                assert "Gestion_Brass_Gestion_Datos.accdb" in str(
                    test_config.db_brass_path
                )
                assert "Tareas_datos1.accdb" in str(test_config.db_tareas_path)
                assert "correos_datos.accdb" in str(test_config.db_correos_path)
    ...
    def test_init_custom_environment_variables(self):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            with patch("common.config.load_dotenv"):
                ...
                assert (
                    test_config.db_brass_path.name
                    == "Gestion_Brass_Gestion_Datos.accdb"
                )
    ...
    def test_init_office_paths(self):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            with patch("common.config.load_dotenv"):
                ...
                assert (
                    r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\Gestion_Brass_Gestion_Datos.accdb"
                    in str(test_config.db_brass_path)
                )
                assert (
                    r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\Tareas_datos1.accdb"
                    in str(test_config.db_tareas_path)
                )
                assert (
                    r"\\datoste\aplicaciones_dys\Aplicaciones PpD\00Recursos\correos_datos.accdb"
                    in str(test_config.db_correos_path)
                )
                assert r"\\datoste\Aplicaciones_dys\Aplicaciones PpD\CSS.txt" in str(
                    test_config.css_file_path
                )
...
def test_default_recipient_telefonica_domain_fallback():
    """Si DEFAULT_RECIPIENT termina en @telefonica.com se fuerza fallback
    'admin@empresa.com'."""
    env_vars = {"ENVIRONMENT": "local", "DEFAULT_RECIPIENT": "soporte@telefonica.com"}
    with patch.dict(os.environ, env_vars, clear=True):
        with patch("common.config.load_dotenv"):
            cfg = Config()
            assert cfg.default_recipient == "admin@empresa.com"
...
def test_get_database_path_correos():
...
def test_get_database_path_invalid():
...
def test_get_db_brass_connection_string():
    ...
    with patch("src.common.config.load_dotenv"):
        ...
        expected = (
            f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ="
            f"{test_config.db_brass_path};PWD=test_pass;"
        )
        assert result == expected


def test_get_db_tareas_connection_string():
    ...
    with patch("src.common.config.load_dotenv"):
        ...
        expected = (
            f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ="
            f"{test_config.db_tareas_path};PWD=test_pass;"
        )
        assert result == expected


def test_get_db_correos_connection_string_with_password():
    ...
    with patch("src.common.config.load_dotenv"):
        ...
        expected = (
            f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ="
            f"{test_config.db_correos_path};PWD=test_pass;"
        )
        assert result == expected


def test_get_db_correos_connection_string_without_password():
    ...
    with patch("src.common.config.load_dotenv"):
        ...
        expected = (
            f"Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ="
            f"{test_config.db_correos_path};"
        )
        assert result == expected


def test_get_db_correos_connection_string_no_password_set():
    """Test generación de cadena de conexión Correos cuando no hay contraseña
    configurada"""
    with patch("src.common.config.load_dotenv"):
        test_config = Config()
        test_config.db_correos_path = Path("test_correos.accdb")
...
def test_smtp_port_conversion():
    """Test conversión de puerto SMTP a entero"""
    env_vars = {"ENVIRONMENT": "local", "LOCAL_SMTP_PORT": "1025"}
