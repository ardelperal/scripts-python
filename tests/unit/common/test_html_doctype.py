...
def fetch_latest_htmls(app, limit=5):
    ...
    with db.get_connection() as conn:
        cur=conn.cursor()
        cur.execute(f"SELECT TOP {limit} Cuerpo FROM TbCorreosEnviados WHERE Aplicacion=? ORDER BY IDCorreo DESC", app)
        rows=cur.fetchall()
        for r in rows:
...
def test_latest_emails_start_with_doctype():
    ...
    for app in apps:
        ...
        for cuerpo in htmls:
            assert cuerpo.lstrip().startswith("<!DOCTYPE html>"), f"Registro {app} no empieza con <!DOCTYPE html>"
