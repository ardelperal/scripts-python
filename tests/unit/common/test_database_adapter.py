...
from src.common.database_adapter import AccessAdapter, PYODBC_AVAILABLE
...
class TestAccessAdapter:
    ...
    def test_init_file_not_found(self):
        ...
        with pytest.raises(FileNotFoundError, match="Base de datos Access no encontrada"):
    ...
    def test_execute_query_with_params(self, mock_db_path, mock_pyodbc_connection):
        ...
        mock_cursor.execute.assert_called_once_with("SELECT * FROM test_table WHERE id = ?", params)
        assert len(result) == 1
    ...
    def test_execute_non_query_success(self, mock_db_path, mock_pyodbc_connection):
        ...
        adapter = AccessAdapter(mock_db_path)
        result = adapter.execute_non_query("INSERT INTO test_table (name) VALUES ('Test')")
    ...
    def test_execute_non_query_with_params(self, mock_db_path, mock_pyodbc_connection):
        ...
        adapter = AccessAdapter(mock_db_path)
        params = ('Test',)
        result = adapter.execute_non_query("INSERT INTO test_table (name) VALUES (?)", params)
        
        mock_cursor.execute.assert_called_once_with("INSERT INTO test_table (name) VALUES (?)", params)
        assert result == 1
...
class TestAccessAdapterImportError:
    ...
    @patch('src.common.database_adapter.pyodbc', None)
    @patch('src.common.database_adapter.PYODBC_AVAILABLE', False)
    def test_import_error_on_module_load(self):
        """Test que se maneja correctamente la falta de pyodbc al cargar el módulo"""
        # Este test verifica que el módulo se puede importar incluso sin pyodbc
        # pero que PYODBC_AVAILABLE es False
        from src.common.database_adapter import PYODBC_AVAILABLE
