...
class TestEmailRecipientsService(unittest.TestCase):
    ...
    @patch('src.common.email.recipients_service._utils.get_admin_emails_string', return_value='a@b;c@d')
    ...
    @patch('src.common.email.recipients_service._utils.get_technical_emails_string', return_value='tec1@test;tec2@test')
    def test_get_technical_emails(self, _mock):
        self.assertEqual(self.service.get_technical_emails(), ['tec1@test', 'tec2@test'])

    def test_get_quality_emails_without_app_id(self):
        self.assertEqual(self.service.get_quality_emails(), [])

    @patch('src.common.email.recipients_service._utils.get_quality_emails_string', return_value='q1@test;q2@test')
