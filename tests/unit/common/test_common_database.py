...
class TestAccessDatabase:
    ...
    def mock_connection_string(self):
        ...
        return "Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=test.accdb;PWD=test;"
    ...
    def test_get_connection_context_manager(self, mock_disconnect, mock_connect, access_db):
    ...
    def test_execute_query_with_params(self, mock_get_connection, access_db):
        ...
        params = (1, 'test')
        result = access_db.execute_query("SELECT COUNT(*) FROM test WHERE id = ? AND name = ?", params)
    ...
    def test_get_max_id(self, mock_execute_query, access_db):
        ...
        mock_execute_query.assert_called_once_with("SELECT MAX(ID) as MaxID FROM TestTable")
        assert result == 42
    ...
    def test_update_record_success(self, mock_execute_non_query, access_db):
        ...
        result = access_db.update_record('TestTable', data, where_condition, where_params)
