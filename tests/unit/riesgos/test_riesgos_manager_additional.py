...
def test_execute_quality_task_no_users_fallback(manager, monkeypatch):
    ...
    monkeypatch.setattr("src.riesgos.riesgos_manager.get_quality_users", lambda *a, **k: [])
    monkeypatch.setattr("src.riesgos.riesgos_manager.get_admin_emails_string", lambda *a, **k: "admin@test")
    monkeypatch.setattr("src.common.utils.register_email_in_database", lambda *a, **k: True)
    monkeypatch.setattr("src.common.utils.register_task_completion", lambda *a, **k: True)
...
def test_normalize_date_variants(manager):
    ...
    assert manager._normalize_date(now.date()) == datetime.combine(now.date(), datetime.min.time())
    assert manager._normalize_date(now.strftime('%Y-%m-%d')) == datetime.strptime(now.strftime('%Y-%m-%d'), '%Y-%m-%d')
...
def test_generate_personalized_quality_report_html(manager):
    ...
    sections = {'UserA': '<div>UserA contenido</div>', 'UserB': '<div>UserB contenido</div>'}
...
def test_generate_quality_member_section_html_empty(manager, monkeypatch):
    # Forzar datos vacíos
    monkeypatch.setattr(manager, '_get_editions_ready_for_publication_data', lambda *a, **k: [])
    monkeypatch.setattr(manager, '_get_editions_about_to_expire_data', lambda *a, **k: [])
    ...
    monkeypatch.setattr(manager, '_get_accepted_risks_to_approve_data', lambda *a, **k: [])
    monkeypatch.setattr(manager, '_get_retired_risks_to_approve_data', lambda *a, **k: [])
    monkeypatch.setattr(manager, '_get_materialized_risks_to_decide_data', lambda *a, **k: [])
...
def test_generate_monthly_quality_report_html_sections(manager, monkeypatch):
    ...
    monkeypatch.setattr(manager, '_get_accepted_risks_pending_approval_data', lambda: [{'Nemotecnico': 'P1'}])
    monkeypatch.setattr(manager, '_get_retired_risks_pending_approval_data', lambda: [{'Nemotecnico': 'P2'}])
    monkeypatch.setattr(manager, '_get_materialized_risks_pending_decision_data', lambda: [{'Nemotecnico': 'P3'}])
    monkeypatch.setattr(manager, '_get_all_editions_ready_for_publication_data', lambda: [{'Nemotecnico': 'P4'}])
    monkeypatch.setattr(manager, '_get_active_editions_data', lambda: [{'Proyecto': 'P5'}])
    monkeypatch.setattr(manager, '_get_closed_editions_last_month_data', lambda: [{'IDEdicion': 1}])
