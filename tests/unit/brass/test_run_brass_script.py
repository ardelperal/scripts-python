...
def test_run_brass_normal(monkeypatch):
    ...
    class DummyTask:
        ...
        def debe_ejecutarse(self): return True
        def execute_specific_logic(self): return True
        def marcar_como_completada(self): self.marked = True
...
def test_run_brass_force(monkeypatch):
    class DummyTask:
        ...
        def __enter__(self): return self
        def __exit__(self, *a): pass
        def debe_ejecutarse(self): return False  # con force se ignora
        def execute_specific_logic(self): self.executed = True; return True
        def marcar_como_completada(self): self.marked = True
    monkeypatch.setattr('src.brass.run_brass.BrassTask', DummyTask)
