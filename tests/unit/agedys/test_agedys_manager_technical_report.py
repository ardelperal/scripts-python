...
def test_technical_report_some_sections():
    ...
    # 2 first queries empty, then dpds_sin_visado empty, rechazados empty, then 4
    # facturas subqueries (only first returns rows)
    mgr = build_manager(
        [
            [],
            [],
...
def test_technical_report_all_sections():
    ...
    # Verificamos que aparece el valor numérico bruto (el formateo de moneda ya no se
    # aplica en el manager moderno)
    assert "1000" in html
