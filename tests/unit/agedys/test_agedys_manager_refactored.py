...
Covers: user aggregation queries, per-user technical report generation, grouped
quality/economy reports, HTML empty report behavior, and section execution logging paths.
"""
...
@pytest.mark.parametrize(
    "subquery_rows",
    [
        # Each element corresponds to rows returned per subquery set inside
        # get_usuarios_facturas_pendientes_visado_tecnico
        [[{"UsuarioRed": "u1", "CorreoUsuario": "u1@acme"}], [], [], []],
        [[], [{"UsuarioRed": "u1", "CorreoUsuario": "u1@acme"}], [], []],
    ],
)
def test_get_usuarios_facturas_pendientes_visado_tecnico_dedup(
    manager, db, subquery_rows
):
...
def test_get_usuarios_facturas_pendientes_visado_tecnico_handles_exceptions(
    manager, db
):
...
def test_generate_quality_report_html(manager):
    manager.get_dpds_sin_visado_calidad_agrupado = Mock(
        return_value=[
            {
                "CODPROYECTOS": "DPD1",
                "PETICIONARIO": "Tec",
...
def test_generate_economy_report_html(manager):
    manager.get_dpds_con_fin_agenda_tecnica_agrupado = Mock(
        return_value=[
            {
                "CODPROYECTOS": "D1",
                "PETICIONARIO": "Tec",
