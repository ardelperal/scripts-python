...
def test_get_dpds_sin_visado_calidad_por_tecnico(manager, db):
    ...
    assert manager.get_dpds_sin_visado_calidad_por_tecnico("User") == [
        {"CODPROYECTOS": "D3"}
    ]
...
def test_get_dpds_rechazados_calidad_por_tecnico(manager, db):
    ...
    assert manager.get_dpds_rechazados_calidad_por_tecnico("User") == [
        {"CODPROYECTOS": "D4"}
    ]
...
def test_get_dpds_con_fin_agenda_tecnica_agrupado(manager, db):
    ...
    assert manager.get_dpds_con_fin_agenda_tecnica_agrupado() == [
        {"CODPROYECTOS": "D6"}
    ]
...
def test_generate_technical_user_report_html_empty(manager, monkeypatch):
    ...
    for name in [
        "get_dpds_sin_so_por_tecnico",
        "get_dpds_con_so_sin_ro_por_tecnico",
        "get_dpds_sin_visado_calidad_por_tecnico",
        "get_dpds_rechazados_calidad_por_tecnico",
    ...
    monkeypatch.setattr(
        gen, "generar_header_moderno", Mock(wraps=gen.generar_header_moderno)
    )
    monkeypatch.setattr(
        gen, "generar_footer_moderno", Mock(wraps=gen.generar_footer_moderno)
    )
...
def test_generate_technical_user_report_html_with_data(manager, monkeypatch):
    monkeypatch.setattr(
        manager,
        "get_dpds_sin_so_por_tecnico",
        Mock(return_value=[{"CODPROYECTOS": "D1"}]),
    )
    monkeypatch.setattr(
...
def test_generate_quality_report_html_with_data(manager, monkeypatch):
    monkeypatch.setattr(
        manager,
        "get_dpds_sin_visado_calidad_agrupado",
        Mock(return_value=[{"CODPROYECTOS": "D1"}]),
    )
    import agedys.agedys_manager as mod
...
def test_generate_economy_report_html_empty(manager, monkeypatch):
    for name in [
        "get_dpds_con_fin_agenda_tecnica_agrupado",
        "get_dpds_sin_pedido_agrupado",
        "get_facturas_rechazadas_agrupado",
        "get_facturas_visadas_pendientes_op_agrupado",
    ]:
        monkeypatch.setattr(manager, name, Mock(return_value=[]))
...
def test_generate_technical_user_report_html_multiple_sections(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        manager,
        "get_dpds_sin_so_por_tecnico",
        Mock(return_value=[{"CODPROYECTOS": "A"}]),
    )
    monkeypatch.setattr(
        manager,
        "get_dpds_con_so_sin_ro_por_tecnico",
        Mock(return_value=[{"CODPROYECTOS": "B"}]),
    )
    monkeypatch.setattr(
        manager,
        "get_dpds_sin_visado_calidad_por_tecnico",
        Mock(return_value=[{"CODPROYECTOS": "C"}]),
    )
    monkeypatch.setattr(
        manager, "get_dpds_rechazados_calidad_por_tecnico", Mock(return_value=[])
    )
...
def test_get_usuarios_con_tareas_pendientes_error_path(manager, db, caplog):
    ...
    db.execute_query.side_effect = [
        ...
        [],
        [],
        [{"UserId": 9, "UserName": "UX", "UserEmail": "ux@x"}],
    ]
    users = manager.get_usuarios_con_tareas_pendientes()
...
def test_get_facturas_pendientes_por_tecnico_error_subquery(manager, db):
    ...
    def exec_side(sql, params=None):
        if (
            "SELECT DISTINCT fd.NFactura" in sql
            and "vf.FRECHAZOTECNICO IS NULL" in sql
            and "er.CorreoSiempre='Sí'" in sql
        ):
