...
def mock_config():
    ...
    with patch('src.agedys.agedys_manager.config') as mock_config:
        mock_config.get_db_agedys_connection_string.return_value = "mock_agedys_connection"
        mock_config.get_db_tareas_connection_string.return_value = "mock_tareas_connection"
        mock_config.get_db_correos_connection_string.return_value = "mock_correos_connection"
...
def mock_database_with_realistic_data():
    ...
    with patch('src.agedys.agedys_manager.AccessDatabase') as mock_db_class:
        ...
        def mock_agedys_query(query, params=None):
            ...
            if 'TbUsuariosAplicaciones.UsuarioRed' in query and 'TbFacturasDetalle' in query:
            ...
            elif 'TbUsuariosAplicaciones.UsuarioRed' in query and 'TbVisadosGenerales' in query:
                return [{'UsuarioRed': 'juan.perez', 'CorreoUsuario': 'juan.perez@empresa.com'}]
            
            # Consultas de DPDs sin visado por usuario
            elif 'TbProyectos.CODPROYECTOS' in query and 'TbVisadosGenerales' in query and params:
                return [dpds_data[0]]  # Primer DPD
            
            # Consultas de usuarios con DPDs rechazados
            elif 'ROFechaRechazo IS NOT NULL' in query:
                return [{'UsuarioRed': 'maria.garcia', 'CorreoUsuario': 'maria.garcia@empresa.com'}]
        ...
        def mock_tareas_query(query, params=None):
            ...
            # Consultas de usuarios técnicos
                ...
                return [u for u in usuarios_data if u['UsuarioRed'] in ['juan.perez', 'carlos.lopez']]
...
def test_complete_business_workflow_facturas_pendientes(agedys_manager, mock_database_with_realistic_data):
    ...
    if usuarios:
        facturas = agedys_manager.get_facturas_pendientes_visado_tecnico(usuarios[0]['UsuarioRed'])
...
def test_user_notification_targeting(agedys_manager, mock_database_with_realistic_data):
    ...
    if usuarios:
        for usuario in usuarios:
            facturas = agedys_manager.get_facturas_pendientes_visado_tecnico(usuario['UsuarioRed'])
...
def test_error_recovery_mechanisms(agedys_manager, mock_database_with_realistic_data):
    ...
    with patch.object(agedys_manager.db, 'execute_query', side_effect=Exception("Database error")):
...
def test_performance_with_large_datasets(agedys_manager, mock_database_with_realistic_data):
...
def test_dry_run_vs_production_differences(agedys_manager, mock_database_with_realistic_data):
...
def test_realistic_end_to_end_scenario(agedys_manager, mock_database_with_realistic_data):
