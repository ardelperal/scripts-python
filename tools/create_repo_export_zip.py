...
Features:
  - Excludes virtual environments, caches, build artifacts, logs, large/local DB files (.accdb, .mdb, .laccdb), coverage outputs, and removed legacy assets.
...
Notes:
  ...
  - If you need to include Access DB schema docs (.md) they will still be included; only the binary DB files are excluded.
"""
...
DEFAULT_EXCLUDES: List[str] = [
    ...
    '__pycache__', '**/__pycache__', '*.py[cod]', '*.pyo', '*.pyd', '*.so',
    # Build / packaging artifacts
    'build', 'dist', 'downloads', 'eggs', '.eggs', '*.egg-info', '*.egg',
    # Test / coverage artifacts
    '.tox', '.pytest_cache', '.pytest_cache/**', 'htmlcov', 'htmlcov/**', '.coverage*', 'coverage.xml', '*.cover',
    # Logs
    'logs', 'logs/**', '*.log',
    # Legacy removed quarantine
    'legacy/.removed', 'legacy/.removed/**',
    # Local database / binary data files
    '*.accdb', '*.mdb', '*.laccdb', 'dbs-locales/*.accdb', 'dbs-locales/*.laccdb',
    # Misc OS / editor
    '.DS_Store', 'Thumbs.db', 'ehthumbs.db',
    # Debug / scratch artifacts referenced previously
    'debug_*.py', 'verify_*.py', 'debug_html', 'debug_html/**', 'quality_output.html', 'setup_local_environment_root.log',
    # Generated exports (avoid nesting self)
    'repo_export_*.zip', 'repo_export_*.tar.gz',
]
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create a curated ZIP of the repository excluding non-relevant artifacts.")
    parser.add_argument('--output', '-o', type=Path, help='Output zip file path (default: repo_export_YYYYMMDD_HHMM.zip in repo root)')
    parser.add_argument('--include-gitignore', action='store_true', help='Also parse .gitignore to extend exclusion patterns')
    parser.add_argument('--extra-exclude', action='append', default=[], help='Additional exclusion glob(s). Can be provided multiple times.')
    parser.add_argument('--force-include', action='append', default=[], help='Glob pattern(s) to force-include even if excluded. Can be repeated.')
    parser.add_argument('--list', action='store_true', help='List included file paths (verbose).')
    parser.add_argument('--dry-run', action='store_true', help='Do not write the zip, only report statistics.')
    parser.add_argument('--show-excludes', action='store_true', help='Print the final exclusion pattern list.')
...
def main(argv: List[str]) -> int:
    ...
    print(f'Included files: {len(included)}  | Excluded paths: {len(excluded)}  | Total size: {human_readable_size(total_size)}')
    if args.list:
        for f in included:
    ...
    print(f'ZIP written: {output_path} (size: {human_readable_size(output_path.stat().st_size)})')
    return 0
